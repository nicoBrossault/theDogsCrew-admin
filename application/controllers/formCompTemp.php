...
class FormCompTemp extends CI_Controller {
	...
	function index(){		
		...
		if(isset($_POST['idcompagnie']) && !empty($_POST['idcompagnie'])){
			...
			$object = $this->doctrine->em->find('Compagnietemp', $id);
			
			$idCompTemp=0;
			$allCompTemp=$this->doctrine->em->getRepository('Compagnietemp')->findAll();
			foreach($allCompTemp as $compTemp){
				if($compTemp->getIdcompagnie()->getIdcompagnie()==$id){
					$idCompTemp=$compTemp->getIdcompagnietemp();
				}
			}
			if($idCompTemp!=0){
				$object = $this->doctrine->em->find('compagnietemp', $idCompTemp);
			}else{
				$object=new Compagnietemp();
			}
			$idComp=$this->doctrine->em->find('compagnie',$_POST['idcompagnie']);
			$object->setIdcompagnie($idComp);
		}
